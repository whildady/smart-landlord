...
<head>
    ...
    <title>Smart Landlord - Document Cloud</title>
    ...
    <div class="flex-1 flex flex-col min-w-0 w-full lg:pl-64">
        <header class="h-16 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/60 flex items-center justify-between px-8 sticky top-0 z-30">
            <div class="flex flex-col text-left">
                <h1 class="text-lg font-bold">Document Cloud</h1>
                <p class="text-[10px] text-slate-400">Secure Vault & File Management.</p>
            </div>
        ...
        <div class="p-8 max-w-7xl mx-auto w-full space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 animate-fade-in-up">
                <div class="relative w-full sm:w-72 md:w-96">
                    <i data-lucide="search" class="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400"></i>
                    <input type="text" id="liveDocSearch" onkeyup="executeLiveDocSearch()" placeholder="Search documents by name or category..." class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl pl-11 pr-4 py-2.5 text-xs font-semibold focus:outline-hidden focus:ring-2 focus:ring-indigo-500/50 transition-all">
                </div>

                <div class="w-full sm:w-auto">
                    <?= Html::beginForm(['landlord/upload-document'], 'post', ['enctype' => 'multipart/form-data', 'id' => 'uploadForm']) ?>
                        <input type="file" name="document_file" id="fileInput" class="hidden" onchange="document.getElementById('uploadForm').submit()">
                        <button type="button" onclick="document.getElementById('fileInput').click()" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-6 rounded-xl text-xs transition-all shadow-md shadow-indigo-600/20 active:scale-95">
                            <i data-lucide="cloud-upload" class="w-4 h-4"></i> Upload File
                        </button>
                    <?= Html::endForm() ?>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" id="documentsContainer">
                <?php if (empty($documents)): ?>
                    <div class="col-span-full py-20 text-center border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl animate-fade-in-up">
                        <i data-lucide="folder-open" class="w-10 h-10 text-slate-300 mx-auto mb-3"></i>
                        <p class="text-slate-400 text-xs">No documents uploaded yet.</p>
                    </div>
                <?php else: ?>
                    <?php foreach ($documents as $doc): ?>
                        <div class="document-item bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800 p-5 rounded-2xl shadow-xs hover:border-indigo-500 transition-all animate-fade-in-up" data-name="<?= strtolower(Html::encode($doc->file_name)) ?>" data-category="<?= strtolower(Html::encode($doc->file_category)) ?>">
                            <div class="flex items-center justify-between mb-4">
                                <i data-lucide="file-text" class="w-8 h-8 text-indigo-500"></i>
                                <span class="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 px-2 py-1 rounded-md uppercase"><?= Html::encode($doc->file_category) ?></span>
                            </div>
                            <h4 class="text-xs font-bold text-slate-900 dark:text-white truncate" title="<?= Html::encode($doc->file_name) ?>">
                                <?= Html::encode($doc->file_name) ?>
                            </h4>
                            <p class="text-[10px] text-slate-400 mt-1 mb-4"><?= date('d M Y', strtotime($doc->created_at)) ?></p>
                            
                            <button type="button" 
        onclick="openViewer('<?= Url::to(['/uploads/documents/' . basename($doc->file_path)]) ?>', '<?= Html::encode($doc->file_name) ?>')" 
        class="w-full text-center py-2 bg-slate-50 dark:bg-slate-800 hover:bg-indigo-600 hover:text-white text-slate-600 dark:text-slate-300 text-[10px] font-bold rounded-xl transition-all">
    <i data-lucide="eye" class="w-3 h-3 inline"></i> View File
</button>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
...
<script>
    function openViewer(url, title) {
        ...
        lucide.createIcons(); // Kuhakikisha icons zinaonekana
    }

    function closeViewerModal() {
        const modal = document.getElementById('viewerModal');
        modal.classList.add('hidden');
        document.getElementById('viewerIframe').src = ""; // Inazuia video/audio kuendelea kucheza nyuma
    }
    ...
    // JavaScript Live Search Engine
    function executeLiveDocSearch() {
        let query = document.getElementById('liveDocSearch').value.toLowerCase();
