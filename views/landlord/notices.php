...
<head>
    ...
    <title>Smart Landlord - Notice Broadcast Studio</title>
...
<body class="bg-[#f8fafc] dark:bg-[#090d16] font-sans antialiased text-slate-900 dark:text-slate-100 flex min-h-screen relative overflow-x-hidden transition-colors duration-300">
    ...
    <!-- MASTER SIDEBAR NAVIGATION (CALENDAR BASE) -->
...
function handleInstagramLogout(event) {
    ...
    <div class="flex-1 flex flex-col min-w-0 w-full lg:pl-64">
        ...
        <!-- HEADER MODULE (WITH PROFILE AND POPUPS) -->
        <header class="h-16 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/60 flex items-center justify-between px-8 sticky top-0 z-30 transition-colors duration-300">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col text-left ml-3">
                    <h1 class="text-lg font-bold text-slate-900 dark:text-white leading-tight tracking-tight">Notice Broadcast Studio</h1>
                    <p class="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">Deploy alerts, push notices, and track message reach rates.</p>
                </div>
            </div>
        ...
        <div class="p-6 max-w-7xl mx-auto w-full space-y-6">
            ...
            <!-- REAL INDEPENDENT CARDS (SEPARATED FOR PREMIUM ANIMATION EFFECTS) -->
            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 p-5 shadow-xs flex flex-col justify-between min-h-[100px] animate-fade-in-up">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="w-8 h-8 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 rounded-xl flex items-center justify-center"><i data-lucide="send" class="w-4 h-4"></i></div>
                    </div>
                    <h3 class="text-2xl font-bold text-slate-800 dark:text-white mt-2"><?= $totalSent ?></h3>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 p-5 shadow-xs flex flex-col justify-between min-h-[100px] animate-fade-in-up" style="animation-delay: 100ms;">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Active Boards</span>
                        <div class="w-8 h-8 bg-violet-50 dark:bg-violet-950/40 text-violet-600 rounded-xl flex items-center justify-center"><i data-lucide="layers" class="w-4 h-4"></i></div>
                    </div>
                    <h3 class="text-2xl font-bold text-indigo-600 dark:text-indigo-400 mt-2"><?= $activeNotices ?></h3>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 p-5 shadow-xs flex flex-col justify-between min-h-[100px] animate-fade-in-up" style="animation-delay: 200ms;">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Avg Reach Rate</span>
                        <div class="w-8 h-8 bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 rounded-xl flex items-center justify-center"><i data-lucide="trending-up" class="w-4 h-4"></i></div>
                    </div>
                    <h3 class="text-2xl font-bold text-emerald-500 mt-2">89.4%</h3>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 p-5 shadow-xs flex flex-col justify-between min-h-[100px] animate-fade-in-up" style="animation-delay: 300ms;">
                    <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">System Gateway</span>
                        <div class="w-8 h-8 bg-teal-50 dark:bg-teal-950/40 text-teal-600 rounded-xl flex items-center justify-center"><i data-lucide="radio" class="w-4 h-4"></i></div>
                    </div>
                    <span class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 mt-3">
                        <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> Connected
                    </span>
                </div>
            </div>

            <!-- MAIN COMPOSER CONTROL WORKSPACE GRID -->
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                
                <!-- COMPOSER ENGINE FRAME (LEFT) -->
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 p-6 shadow-xs h-fit animate-fade-in-up">
                    <div class="flex flex-col text-left mb-5">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-white">Compose Broadcast</h3>
                        <p class="text-[11px] text-slate-400 mt-0.5">Select audience scope and configure delivery matrix.</p>
                    </div>

                    <?= Html::beginForm(['landlord/notices'], 'post', ['class' => 'space-y-4']) ?>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Notice Title</label>
                            <input type="text" name="title" required placeholder="e.g. Emergency Water Interruption" class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs font-semibold focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                        </div>

                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Target Scope / Property</label>
                            <select name="audience_type" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                                <option value="All">Broadcast to All Buildings/Tenants</option>
                                <?php if (!empty($properties)): ?>
                                    <?php foreach ($properties as $prop): ?>
                                        <!-- REAL DATA INTEGRATION: Loading actual properties dynamic database bounds -->
                                        <option value="<?= Html::encode($prop['name']) ?>">Building: <?= Html::encode($prop['name']) ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Severity Level</label>
                                <select name="severity" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-hidden">
                                    <option value="Informational">Informational (Blue)</option>
                                    <option value="Warning">Warning (Amber)</option>
                                    <option value="Critical">Critical Alert (Red)</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Expiry Date Bound</label>
                                <input type="date" name="expiry_date" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs focus:outline-hidden">
                            </div>
                        </div>

                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Notice Content</label>
                            <textarea name="content" rows="4" required placeholder="Type the broadcast message context here..." class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden"></textarea>
                        </div>

                        <div class="bg-slate-50 dark:bg-slate-800/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800 space-y-2">
                            <span class="block text-[10px] font-bold uppercase tracking-wider text-slate-400">Dispatch Gateways</span>
                            <div class="flex items-center gap-4 text-xs">
                                <label class="flex items-center gap-1.5 font-medium cursor-pointer select-none">
                                    <input type="checkbox" name="channels[]" value="App" checked class="rounded text-indigo-600 focus:ring-0"> App Feed
                                </label>
                                <label class="flex items-center gap-1.5 font-medium cursor-pointer select-none">
                                    <input type="checkbox" name="channels[]" value="SMS" class="rounded text-indigo-600 focus:ring-0"> Push SMS
                                </label>
                                <label class="flex items-center gap-1.5 font-medium cursor-pointer select-none">
                                    <input type="checkbox" name="channels[]" value="Email" class="rounded text-indigo-600 focus:ring-0"> Email PDF
                                </label>
                            </div>
                        </div>

                        <button type="submit" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl text-xs transition-all cursor-pointer shadow-sm active:scale-95">
                            Launch Notice Stream
                        </button>
                    <?= Html::endForm() ?>
                </div>

                <!-- REAL INTERACTIVE BROADCAST LIVE STREAM BOARD (RIGHT) -->
                <div class="xl:col-span-2 space-y-4">
                    <div class="flex items-center justify-between px-2">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-white">Live Bulletin Board Stream</h3>
                        <span class="text-xs text-slate-400 font-medium tracking-tight">Active system feeds</span>
                    </div>

                    <div class="space-y-4 overflow-y-auto max-h-[640px] pr-1 sidebar-scroll" id="bulletinContainer">
                        <?php if (empty($noticesList)): ?>
                            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 p-12 text-center text-slate-400 text-xs font-medium">
                                No active notifications launched on the dashboard matrix yet.
                            </div>
                        <?php else: ?>
                            <?php foreach ($noticesList as $notice): ?>
                                <?php 
                                    $isExpired = strtotime($notice['expiry_date']) < time();
                                    $colorMap = [
                                        'Critical' => 'border-l-rose-500 bg-rose-500/[0.01]',
                                        'Warning' => 'border-l-amber-500 bg-amber-500/[0.01]',
                                        'Informational' => 'border-l-blue-500 bg-blue-500/[0.01]'
                                    ];
                                    $badgeMap = [
                                        ...
                                        'Informational' => 'bg-blue-50 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400'
                                    ];
                                    $cls = $colorMap[$notice['severity']] ?? 'border-l-slate-300';
                                    $badgeCls = $badgeMap[$notice['severity']] ?? 'bg-slate-100';
                                ?>
                                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 border-l-4 <?= $cls ?> p-5 shadow-xs relative animate-fade-in-up">
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="text-left">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span class="text-[9px] font-bold uppercase px-2 py-0.5 rounded-sm <?= $badgeCls ?>"><?= $notice['severity'] ?></span>
                                                <span class="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Scope: <?= Html::encode($notice['audience_type']) ?></span>
                                            </div>
                                            <h4 class="text-sm font-bold text-slate-900 dark:text-white mt-2"><?= Html::encode($notice['title']) ?></h4>
                                        </div>
                                        <span class="text-[10px] font-bold text-slate-400 whitespace-nowrap"><?= date('M d, Y', strtotime($notice['created_at'])) ?></span>
                                    </div>
                                    
                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-2 text-left leading-relaxed"><?= Html::encode($notice['content']) ?></p>
                                    
                                    <div class="mt-4 pt-4 border-t border-slate-50 dark:border-slate-800/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        <div class="flex items-center gap-3">
                                            <span class="text-[10px] text-slate-400 font-bold uppercase">Active Channels:</span>
                                            <div class="flex items-center gap-2 text-slate-400">
                                                <?php $ch = explode(',', $notice['channels']); ?>
                                                <i data-lucide="smartphone" class="w-4 h-4 <?= in_array('App', $ch) ? 'text-indigo-500' : 'opacity-20' ?>" title="Portal Feed App"></i>
                                                <i data-lucide="message-square" class="w-4 h-4 <?= in_array('SMS', $ch) ? 'text-indigo-500' : 'opacity-20' ?>" title="SMS Carrier Gateway"></i>
                                                <i data-lucide="mail" class="w-4 h-4 <?= in_array('Email', $ch) ? 'text-indigo-500' : 'opacity-20' ?>" title="SMTP Email Routing"></i>
                                            </div>
                                        </div>
                                        
                                        <div class="flex items-center gap-2 text-[10px] font-bold text-slate-400">
                                            <span>Expires:</span>
                                            <span class="<?= $isExpired ? 'text-rose-500 font-extrabold' : 'text-slate-600 dark:text-slate-300' ?>"><?= $notice['expiry_date'] ?> <?= $isExpired ? '(Expired)' : '' ?></span>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; ?>
