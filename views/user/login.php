...
<head>
    ...
    <title>Sign In | Smart Landlord Premium</title>
    <!-- Tailwind CSS v4 -->
...
<body class="bg-white font-sans antialiased min-h-screen flex text-slate-800">
    ...
    <div class="w-full min-h-screen flex flex-col md:flex-row bg-white">
        ...
        <!-- ================= UPANDE WA KUSHOTO: HIGH-CONVERTING ADVERTISEMENT BANNER ================= -->
        <div class="w-full md:w-1/2 min-h-[550px] md:min-h-screen dotted-bg relative flex flex-col justify-between p-8 lg:p-16 border-b md:border-b-0 md:border-r border-slate-100">
            ...
            <!-- Main Marketing & Slider Section -->
            <div class="w-full max-w-lg mx-auto my-auto space-y-8 z-20">
                
                <!-- dynamic text content holder (Inabadilishwa na JavaScript) -->
                ...
                <!-- PROFESSIONAL MULTI-MEDIA CONTAINER -->
                <div class="relative w-full h-72 sm:h-80 bg-slate-50 rounded-3xl p-3 border border-slate-200/80 shadow-2xl shadow-slate-200/50 overflow-hidden">
                    ...
                    <!-- Slide Progress Indicators (Nukta ndogo za chini kwenye bango) -->
                ...
                <!-- TRUSTED METRICS (Hapa ndio siri ya kupiga pesa kibiashara) -->
        ...
        <!-- <!-- ================= UPANDE WA KULIA: ENTERPRISE LIGHT LOGIN FORM ================= -->
        <div class="w-full md:w-1/2 flex items-center justify-center p-6 sm:p-12 md:p-16 bg-white z-20">
            <div class="w-full max-w-sm space-y-8">
                
                <!-- Nembo ya Mfumo na Kichwa cha Habari (Safi na haina mbwembwe nyingi) -->
               <div class="space-y-2">
        ...
        <div class="flex flex-col justify-center">
            ...
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 transition-all duration-300 group-hover:tracking-wider group-hover:text-indigo-600">
                Premium Ecosystem
                    ...
                    <p class="text-sm text-slate-500 font-medium">Welcome back! Please enter your details below.</p>
                </div>

                <!-- Form ya Yii2 iliyosafishwa (Borderless kama image_951eff.jpg) -->
                <form action="<?= Url::to(['/user/login']) ?>" method="POST" class="space-y-4">
                    ...
                    <div class="flex justify-between items-center text-xs sm:text-sm pt-0.5">
                        <label class="flex items-center gap-2 text-slate-500 cursor-pointer select-none font-medium">
                            <input type="checkbox" name="rememberMe" class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 accent-indigo-600 cursor-pointer">
                            <span>Remember me</span>
                        </label>
                        <a href="<?= Url::to(['/user/forgot-password']) ?>" class="font-bold text-slate-600 hover:text-slate-900 transition-colors">Forgot password?</a>
                    </div>

                    <!-- Kitufe cha Kisasa cha Kuendelea (Sio cheusi kabisa lakini ni Indigo Nzito ya Kifalme) -->
                ...
                <!-- VITUFE VYA HARAKA VYA KIUSALAMA (Google & Apple Kama mfano wa picha yako) -->
                <div class="space-y-3">
...
document.getElementById('google-auth-btn').addEventListener('click', function(e) {
    // 1. Tafuta dropdown ya kuchagua jukumu (Mfano: Tenant au Landlord)
    // KUMBUKA: Kama id ya dropdown yako sio 'account-type', badilisha jina hili hapa chini liendane na id halisi ya dropdown yako!
    var roleSelect = document.getElementById('account-type'); 
    var chosenRole = roleSelect ? roleSelect.value.toLowerCase() : 'tenant';
    
    // 2. Chukua link iliyotengenezwa na Yii2 Url::to
    var currentHref = this.getAttribute('href');
    
    // 3. Inaongeza parameter ya role kitalamu ili controller yetu kule nyuma iisome
    if (currentHref.indexOf('role=') === -1) {
        this.setAttribute('href', currentHref + '&role=' + chosenRole);
    }
});
</script>
    
   
                <!-- Kiungo cha Jisajili chini -->
                <div class="text-center pt-2">
                    <p class="text-sm text-slate-500">
                        Don't have an account yet? 
                        <a href="<?= Url::to(['/user/register']) ?>" class="font-bold text-slate-900 hover:text-indigo-600 transition-colors ml-1">Sign up</a>
                    </p>
                </div>
            </div>
        </div>

    <!-- JAVASCRIPT LOGIC FOR HIGH-END MARKETING ENGINE -->
    <script>
        lucide.createIcons();
        ...
        function updateSlideshow() {
            ...
            // Fade out maandishi kidogo kabla ya kubadilika
            titleEl.classList.add('opacity-0', '-translate-y-1');
            descEl.classList.add('opacity-0');

            // 2. Nenda index inayofuata
            currentIndex = (currentIndex + 1) % images.length;

            // 3. Washa picha na nukta mpya
            ...
            indicators[currentIndex].classList.add('w-4'); // fanya nukta inayofanya kazi iwe ndefu kidogo

            // 4. Badilisha maandishi na yaonyeshe kwa ulainifu
            setTimeout(() => {
...
document.addEventListener("DOMContentLoaded", function() {
    // Angalia kama kuna login info iliyosaviwa
    const savedEmail = localStorage.getItem('saved_tenant_email');
    const rememberMe = localStorage.getItem('remember_me');

    if (rememberMe === 'true' && savedEmail) {
        // Tafuta input ya email au username kwenye form yako ya login ya Yii2
        // Kawaida Yii2 inatumia id kama "loginform-username" au "loginform-email"
        const emailInput = document.querySelector('input[type="email"]') || document.getElementById('loginform-username');
        const rememberCheckbox = document.getElementById('loginform-rememberme');

        if (emailInput) {
            emailInput.value = savedEmail;
            
            // Weka alama ya tiki kwenye kadi ya "Remember Me" ya Yii2 kiotomatiki
            if (rememberCheckbox) {
                rememberCheckbox.checked = true;
            }
            
            console.log("Login info loaded automatically for: " + savedEmail);
        }
    }
});
</script>
