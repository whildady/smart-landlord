...
function handleInstagramLogout(event) {
    ...
    <div class="flex-1 flex flex-col min-w-0 w-full lg:pl-64">
        ...
        <header class="h-16 bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/60 flex items-center justify-between px-8 sticky top-0 z-30 transition-colors duration-300">
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col text-left ml-3">
                    <h1 class="text-lg font-bold text-slate-900 dark:text-white leading-tight tracking-tight">Expenses Tracker</h1>
                    <p class="text-xs font-medium text-slate-400 dark:text-slate-500 mt-1">Track building and operational costs.</p>
                </div>
            </div>
        ...
        <div class="p-6 max-w-7xl mx-auto w-full space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 pt-5 px-5 pb-0 shadow-xs relative overflow-hidden flex flex-col justify-between min-h-[140px] animate-fade-in-up">
                    <div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Expenses (This Month)</span>
                            <div class="w-8 h-8 bg-rose-50 dark:bg-rose-950/40 text-rose-600 rounded-xl flex items-center justify-center font-bold text-xs">Tsh</div>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white mt-2">Sh <?= number_format($totalExpensesMonth) ?></h3>
                    </div>
                    <div class="w-full h-8"><canvas id="expenseMiniChart1"></canvas></div>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 pt-5 px-5 pb-0 shadow-xs relative overflow-hidden flex flex-col justify-between min-h-[140px] animate-fade-in-up">
                    <div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Unpaid Vendor Bills</span>
                            <div class="w-8 h-8 bg-amber-50 dark:bg-amber-950/40 text-amber-600 rounded-xl flex items-center justify-center"><i data-lucide="clock" class="w-4 h-4"></i></div>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white mt-2">Sh <?= number_format($pendingApprovals) ?></h3>
                    </div>
                    <div class="w-full h-8"><canvas id="expenseMiniChart2"></canvas></div>
                </div>

                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 pt-5 px-5 pb-0 shadow-xs relative overflow-hidden flex flex-col justify-between min-h-[140px] animate-fade-in-up">
                    <div>
                        <div class="flex items-center justify-between">
                            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Operational Cost Ratio</span>
                            <div class="w-8 h-8 bg-teal-50 dark:bg-teal-950/40 text-teal-600 rounded-xl flex items-center justify-center"><i data-lucide="percent" class="w-4 h-4"></i></div>
                        </div>
                        <h3 class="text-xl font-bold text-slate-800 dark:text-white mt-2"><?= $costRatio ?>% <span class="text-xs text-slate-400 font-normal">of income</span></h3>
                    </div>
                    <div class="w-full h-8"><canvas id="expenseMiniChart3"></canvas></div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 p-6 shadow-xs animate-fade-in-up">
                    <div class="flex flex-col text-left mb-6">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-white">Record New Expense</h3>
                        <p class="text-[11px] text-slate-400 mt-0.5">Enter business costs dynamically synced with MySQL.</p>
                    </div>

                    <?= Html::beginForm(['landlord/expenses'], 'post', ['class' => 'space-y-4']) ?>
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Select Property / Building</label>
                            <select name="property_id" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                                <option value="">-- Choose Property --</option>
                                <?php foreach ($properties as $prop): ?>
                                    <option value="<?= $prop['id'] ?>"><?= Html::encode($prop['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>

                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Expense Category</label>
                            <select name="category" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                                <option value="Maintenance (Repairs)">Maintenance (Plumbing, Electrical, Painting)</option>
                                <option value="Utilities (LUKU/Water)">Utilities (Water, Public Space Electricity)</option>
                                <option value="Security & Cleaning">Security & Cleaning</option>
                                <option value="Taxes & Government (TRA)">Taxes & Government (TRA Property Rate, Land Rent)</option>
                                <option value="Administration & Salaries">Administration & Salaries (Staff Salaries)</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Amount (Tsh)</label>
                            <div class="relative">
                                <span class="absolute left-4 top-2.5 text-xs text-slate-400 font-bold">Sh</span>
                                <input type="number" name="amount" min="1" placeholder="Example: 250,000" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl pl-10 pr-4 py-2.5 text-xs font-bold focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                            </div>
                        </div>

                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Payment Method</label>
                            <select name="payment_method" required class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500">
                                <option value="Mobile Money (M-Pesa/TigoPesa)">Mobile Money (M-Pesa, Tigo, Airtel)</option>
                                <option value="Bank Transfer (NMB/CRDB)">Bank Transfer (CRDB / NMB)</option>
                                <option value="Cash">Cash</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1">Expense Description / Notes</label>
                            <textarea name="description" rows="2" placeholder="Expense breakdown details (e.g., Paying current month water utility bill)" class="w-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-xs focus:outline-hidden focus:ring-2 focus:ring-indigo-500"></textarea>
                        </div>
                ...
                <div class="xl:col-span-2 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 p-6 shadow-xs flex flex-col justify-between animate-fade-in-up">
                    <div class="flex items-center justify-between mb-6">
                        <div class="flex flex-col text-left">
                            <h3 class="text-sm font-bold text-slate-800 dark:text-white">Expense Statement</h3>
                            <p class="text-[11px] text-slate-400 mt-0.5">Yearly expense trends compiled dynamically.</p>
                        </div>
                    </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800/60 p-6 shadow-xs animate-fade-in-up">
                <div class="flex items-center justify-between mb-6">
                    <div class="flex flex-col text-left">
                        <h3 class="text-sm font-bold text-slate-800 dark:text-white">Transaction Ledger</h3>
                        <p class="text-[11px] text-slate-400 mt-0.5">Historical logs of all disbursements.</p>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="text-xs divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-600 dark:text-slate-300">
                            <?php if (empty($expensesList)): ?>
                                <tr>
                                    <td colspan="6" class="text-center py-8 text-slate-400 font-medium">No expenses recorded yet.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($expensesList as $exp): ?>
                                    <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-all">
                                        <td class="py-3.5 px-4 font-semibold"><?= Html::encode($exp['expense_date'] ?? date('Y-m-d')) ?></td>
                                        <td class="py-3.5 px-4 font-medium text-slate-900 dark:text-white"><?= Html::encode($exp['property_name'] ?? 'General') ?></td>
                                        <td class="py-3.5 px-4"><?= Html::encode($exp['category']) ?></td>
                                        <td class="py-3.5 px-4 text-slate-400"><?= Html::encode($exp['payment_method']) ?></td>
                                        <td class="py-3.5 px-4 font-bold text-slate-900 dark:text-white">Sh <?= number_format($exp['amount']) ?></td>
                                        <td class="py-3.5 px-4">
                                            <span class="px-2.5 py-1 rounded-lg text-[10px] font-bold <?= $exp['status'] === 'Paid' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400' : 'bg-amber-50 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400' ?>">
                                                <?= Html::encode($exp['status']) ?>
                                            </span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        new Chart(document.getElementById('expenseMiniChart1').getContext('2d'), {
            type: 'line',
            data: { labels: [1,2,3,4,5,6], datasets: [{ data: [15,18,12,24,19,21], borderColor: '#f43f5e', backgroundColor: 'transparent' }] },
            options: miniOptions
        });

        new Chart(document.getElementById('expenseMiniChart2').getContext('2d'), {
            type: 'line',
            data: { labels: [1,2,3,4,5,6], datasets: [{ data: [10,12,14,11,16,15], borderColor: '#d97706', backgroundColor: 'transparent' }] },
            options: miniOptions
        });

        new Chart(document.getElementById('expenseMiniChart3').getContext('2d'), {
            type: 'line',
            data: { labels: [1,2,3,4,5,6], datasets: [{ data: [5,8,12,7,9,14], borderColor: '#0d9488', backgroundColor: 'transparent' }] },
            options: miniOptions
        });

        const yearlyCtx = document.getElementById('expenseYearlyChart').getContext('2d');
        const expGradient = yearlyCtx.createLinearGradient(0, 0, 0, 250);
        expGradient.addColorStop(0, 'rgba(244, 63, 94, 0.22)');
